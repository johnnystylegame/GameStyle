...
function theme_sidebar($args) {
    ...
    foreach ($widgets as $widget) {
        $widgetHtml = strtr($args['template'], array(
...
function theme_404_content($args = '') {
    $args = wp_parse_args($args, array(
        'error_title' => __('Nothing here', 'site1'),
        'error_message' => __('It looks like nothing was found at this location. Maybe try a search?', 'site1')
    ));
    extract($args);
